...
// 开发语言：C++ / EasyX 图形库
// 游戏类型：悬疑推理/解谜
// 游戏背景：玩家扮演侦探，调查古堡内的谋杀案
// 主要功能：现场勘查、询问嫌疑人、收集线索、指认真凶
//-----------------------------------------------------------------------------
...
enum Suspect {
    NONE,            // 无（初始状态）
    SUSPECT_WIFE,    // 嫌疑人：男爵夫人
    SUSPECT_JACK,    // 嫌疑人：养子杰克
    SUSPECT_BUTLER   // 嫌疑人：老管家
};
...
struct DialogOption {
    TCHAR text[100];      // 问题文本
    TCHAR response[500];  // 回答内容
    bool requiresClue;    // 是否需要特定线索才能解锁此问题
    TCHAR requiredClue[50]; // 需要的线索名称
    bool revealsClue;     // 是否揭示新线索
    TCHAR newClue[200];   // 新线索内容
};

//-----------------------------------------------------------------------------
// 函数前向声明
//-----------------------------------------------------------------------------
void SetChineseFont(int height);                                      // 设置中文字体
void LoadImages();                                                   // 加载所有图片资源
void DrawTitleScreen();                                              // 绘制标题画面
void DrawIntroScreen();                                              // 绘制剧情介绍画面
void DrawGameScreen();                                               // 绘制主游戏画面
void ShowClue(const TCHAR* title, const TCHAR* desc);                // 显示线索详情
void DrawQuestionScreen();                                           // 绘制询问问题画面
void DrawDialogScreen();                                             // 绘制对话画面
void DrawAccuseScreen();                                             // 绘制指认画面
void DrawEndingScreen(bool isWin);                                   // 绘制结局画面
void HandleMouseClick(int x, int y);                                 // 处理鼠标点击事件
void StartTransition(GameState newState);                            // 开始场景过渡
void UpdateTransition();                                             // 更新过渡效果
void DrawTransition();                                               // 绘制过渡效果
void DrawProgressBar(int x, int y, int width, int height, int current, int max); // 绘制进度条
void PlaySoundEffect(const TCHAR* soundName);                        // 播放音效
...
void DrawBackgroundStoryScreen();                                     // 绘制人物背景故事画面

//-----------------------------------------------------------------------------
// 图片资源全局变量
//-----------------------------------------------------------------------------
IMAGE imgBackground;   // 背景图片
IMAGE imgWife;         // 男爵夫人立绘
IMAGE imgJack;         // 养子杰克立绘
IMAGE imgButler;       // 老管家立绘
IMAGE imgCandle;       // 烛台（凶器）
IMAGE imgLetter;       // 信件和遗嘱草稿
IMAGE imgScarf;        // 女士羊毛围巾
IMAGE imgFootprint;    // 泥脚印
IMAGE imgBody;         // 男爵尸体
IMAGE imgDialogBox;    // 对话框背景
IMAGE imgTitle;        // 游戏标题图片
IMAGE imgAccuseBtn;    // 指认按钮
IMAGE imgWinEnd;       // 胜利结局背景
IMAGE imgLoseEnd;      // 失败结局背景
...
// 线索收集状态
//-----------------------------------------------------------------------------
bool clueCandle = false;    // 烛台线索是否已收集
bool clueLetter = false;    // 信件遗嘱线索是否已收集
bool clueScarf = false;     // 围巾线索是否已收集
bool clueBody = false;      // 尸体线索是否已收集
bool clueFootprint = false; // 脚印线索是否已收集
int keyClueCount = 0;       // 关键线索收集数量

//-----------------------------------------------------------------------------
// 询问进度状态
//-----------------------------------------------------------------------------
bool wifeInterrogated = false;   // 男爵夫人是否已询问
bool jackInterrogated = false;   // 养子杰克是否已询问
bool butlerInterrogated = false; // 老管家是否已询问

// 各嫌疑人的问题询问状态
bool wifeQuestion1 = false;  // 夫人问题1是否已问
bool wifeQuestion2 = false;  // 夫人问题2是否已问
bool wifeQuestion3 = false;  // 夫人问题3是否已问
bool jackQuestion1 = false;  // 杰克问题1是否已问
bool jackQuestion2 = false;  // 杰克问题2是否已问
bool jackQuestion3 = false;  // 杰克问题3是否已问
bool butlerQuestion1 = false; // 管家问题1是否已问
bool butlerQuestion2 = false; // 管家问题2是否已问
bool butlerQuestion3 = false; // 管家问题3是否已问

//-----------------------------------------------------------------------------
// 场景过渡状态
//-----------------------------------------------------------------------------
bool isTransitioning = false; // 是否正在进行场景过渡
int transitionAlpha = 255;    // 过渡效果的透明度（0-255）

//-----------------------------------------------------------------------------
// 鼠标状态
//-----------------------------------------------------------------------------
int mouseX = 0, mouseY = 0;       // 当前鼠标位置
bool isHoveringButton = false;     // 是否悬停在可交互按钮上
...
void UpdateTransition() {
    ...
    transitionAlpha += 15;  // 增加透明度
    if (transitionAlpha >= 255) {
        transitionAlpha = 255;
        currentState = nextState;  // 切换到新场景
...
// 绘制进度条
// 参数：x, y - 进度条位置；width, height - 进度条尺寸；current - 当前值；max - 最大值
...
void DrawTitleScreen() {
    cleardevice();  // 清空画布
    putimage(0, 0, &imgBackground);  // 绘制背景
    ...
    outtextxy(500, 680, _T("© 2024 Mystery Castle"));
}
...
void ShowClue(const TCHAR* title, const TCHAR* desc) {
    ...
    outtextxy(titleX, innerY+50 , title);
